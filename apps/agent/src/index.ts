...
export const executePrompt = async (prompt: string, {host, port}: {host: string, port: number}) => {
    ...
    const { object: classification } = await generateObject({
        ...
        schema: z.object({
          ...
          additionalGroups: z.array(z.enum(['crypto', 'general', 'computer'])).optional(),
    ...
    const groupsToUse = useMultipleGroups && additionalGroups.length > 0
    ...
    // Use multi-group selection if we have multiple groups, otherwise use single group
    const toolsToUse = uniqueGroups.length > 1 
...
`;
    ...
    const result = await generateText({
        ...
        system: "You are an AI assistant that follows plans methodically. Work through the steps in the plan to complete the user's task.",
    ...
    for (const toolCall of result.toolCalls) {  
        ...
        const matchingSteps = plan.steps.filter(
            step => step.suggestedTools?.includes(toolCall.toolName)
        );
    ...
    const percentComplete = Math.round((completedSteps.length / plan.steps.length) * 100);
    redisClient.set("plan_progress", JSON.stringify({
