...
export class VNCComputer implements Computer {
    ...
    try {
      return new Promise<void>((resolve, reject) => {
        ...
        this.client.on('connect', () => {
          ...
          console.log(
            `Remote screen name: ${this.client.title}, width: ${this.client.width}, height: ${this.client.height}`
          );

          // Update dimensions based on remote screen
          this.dimensions = [
            this.client.width || this.dimensions[0],
            this.client.height || this.dimensions[1],
          ];
          this.connected = true;
        ...
        this.client.on('resize', (rect: any) => {
          ...
          this.dimensions = [
            rect.width || this.dimensions[0],
            rect.height || this.dimensions[1],
          ];
  ...
  private handleRawRect(rect: any) {
    try {
      ...
      console.log(
        `Received raw rect: ${x},${y} ${width}x${height} with ${data.length} bytes of data`
      );
  ...
  private requestScreenUpdate(incremental = true) {
    ...
    try {
      console.log(
        `Requesting ${incremental ? 'incremental' : 'full'} screen update`
      );
  ...
  private createDummyImage(
    message = 'VNC Connection Active - No Frame Data'
  ): string {
  ...
  private async convertFrameBufferToImage(buffer: Buffer): Promise<string> {
    try {
      ...
      if (buffer.length < expectedSize) {
        console.warn(
          `Frame buffer size (${buffer.length}) is smaller than expected (${expectedSize})`
        );
      ...
      return this.createDummyImage(
        `Image conversion error: ${error.message || 'Unknown error'}`
      );
    }
  }
  ...
  async doubleClick(x: number, y: number, button = 'left'): Promise<void> {
    try {
  ...
  async drag(path: Array<{ x: number; y: number }>): Promise<void> {
    ...
    try {
      console.log(
        `Dragging from (${path[0]?.x}, ${path[0]?.y}) to (${path[path.length - 1]?.x}, ${path[path.length - 1]?.y})`
      );
